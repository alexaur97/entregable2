...
public interface RequestRepository extends JpaRepository<Request, Integer> {
	...
	// FR 10.6 (a)
	@Query("select r from Request r where r.status = ?1 and r.parade.brotherhood.id = ?2")
	Collection<Request> findRequestByStatusAndBrotherhood(String status, int id);

	// FR 10.6 (b)
	@Query("select r from Request r where r.parade.brotherhood.id = ?1 group by status")
	...
	@Query("select r from Request r where r.parade.id = ?1 and r.status=?2 ")
	Collection<Request> findRequestApprovedByParade(int id, String status);
